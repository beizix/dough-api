...
public class SaveFileService implements SaveFileUseCase {
  ...
  public Optional<SaveFile> operate(
    ...
    try (InputStream bis = new BufferedInputStream(inputStream)) {
      ...
      throw new RuntimeException(String.format("[%s] 파일 처리 중 예상치 못한 오류가 발생했습니다.", originalFilename), e);
    }
  }
  ...
  private void validateFile(
    ...
    AcceptableFileType fileType = getFileTypeMatchingExtension(fileUploadType, extension)
      .orElseThrow(
        () -> new IllegalArgumentException(String.format("'.%s' - 허용되지 않는 파일 확장자입니다.", extension)));

    // ✦ 파일 mime-type 체크
    String fileMimeType = tika.detect(inputStream, originalFilename);
    getMimeType(fileType, fileMimeType)
      .orElseThrow(
        () -> new IllegalArgumentException(String.format("'%s' - 허용되지 않는 MIME Type 입니다.", fileMimeType)));
  }
  ...
  private SaveToFileStorage getFileUploadStrategy(FileStorageType fileStorageType) {
    return fileUploadStrategies.stream()
      .filter(
        saveToFileStorage -> saveToFileStorage.getStorageType().equals(fileStorageType))
      .findFirst()
      .orElseThrow(
