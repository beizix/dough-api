...
public class UploadFileWebAdapter {
    ...
    @ApiResponse(responseCode = "200", description = "업로드 성공", content = @Content(schema = @Schema(implementation = UploadFileResponse.class)))
    @PostMapping(value = "/multipart", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
    public UploadFileResponse uploadMultipart(@Valid @ModelAttribute UploadMultipartRequest req)
            throws IOException {

        SaveFile result = saveFileUseCase
                .operate(
                        req.type(),
                        req.file().getInputStream(),
                        req.file().getOriginalFilename(),
                        req.file().getSize())
                .orElseThrow(() -> new RuntimeException(messageUtils.getMessage("exception.file.upload_failed")));
